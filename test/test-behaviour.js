...
function spawnAndCollectStderr(cmd, args, opts, done) {
  ...
  var spawnedProcess = childProcess.spawn(cmd, args, opts);
...
describe('bespoke generator', function () {
  ...
  before(function () {
    ...
    this.ctx = helpers.run(path.join(__dirname, '../app'))
      .inTmpDir(function (dir) {
        ...
        childProcess.execSync('git remote add origin git@github.com:presenter/presentation-foobar.git'); 
      })
  ...
  it('should populate the repository field in package.json if called within a git repository', function (done) {
    assert.jsonFileContent('package.json', {
      repository: {
        type: 'git',
        url: 'https://github.com/presenter/presentation-foobar.git'
      }
    });
    return done(null);
  });
